...
// 飲食店の全件表示する機能
function createAllCard(card_id,restaurant_post_id,restaurant_title,restaurant_name, restaurant_comment, restaurant_address, restaurant_start_time, restaurant_finish_time, restaurant_budget, restaurant_category_id, restaurant_date, restaurant_release){
    
}

// 観光名所投稿のカードを生成する機能
function createTouristCard(parent,tourist_spot_id, tourist_spot_name,tourist_spot_address,tourist_spot_start,tourist_spot_end,tourist_spot_title,tourist_spot_comment,category_id,plan_spot_day,tourist_release){
    // console.log(parent);
    // 新しい子要素1を作成
    const newCard = document.createElement("div");
    newCard.className = "card-ele";
    newCard.style.width = "100%";
    newCard.innerHTML = `
        <img id=`+tourist_spot_id+` src="./images/homeback.png" class="img-top" alt="...">
        <div class="text-body">
            <h4 class="text">`+tourist_spot_title+`</h4>
            <p>`+tourist_spot_comment+`</p>
        </div>`;
    // 新しい子要素を親要素に追加
    parent.appendChild(newCard);
}
